...
describe('slugify', () => {
  ...
  it('handles multiple spaces and hyphens', () => {
    expect(slugify('hello   world---test')).toBe(
      'hello-world-test'
    );
  });
...
describe('cn (className utility)', () => {
  ...
  it('filters out falsy values', () => {
    expect(
      cn('foo', false, 'bar', undefined, null)
    ).toBe('foo bar');
  });

  it('handles object input', () => {
    expect(
      cn('foo', { bar: true, baz: false })
    ).toBe('foo bar');
  });
...
describe('getInitials', () => {
  it('returns initials from full name', () => {
    expect(getInitials('Manju L')).toBe('ML');
  });

  it('handles single name', () => {
    expect(getInitials('Manju')).toBe('M');
  });

  it('handles multiple names', () => {
    expect(getInitials('Manju Lakshmi Gowda')).toBe('ML');
  });
});
